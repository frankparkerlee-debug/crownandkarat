...
export default function Testimonials() {
  return (
    <section className="section-container bg-surface-light">
      <SectionHeading
        tag="Reviews"
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {testimonials.map((testimonial, index) => (
          <div
            key={index}
            className="bg-surface p-8 rounded-xl border border-white/5"
          >
            {/* Stars */}
            ...
            <div className="border-t border-white/10 pt-4">
              ...
              <p className="text-sm text-cream-muted">{testimonial.location}</p>
              <p className="text-xs text-gold mt-1">Sold: {testimonial.item}</p>
            </div>
          </div>
        ))}
      </div>
